---
name: game
description: "Smart router for game projects. Auto-detects what to do based on context — new game / analyze existing / continue work / UI redesign / pre-release. One command, AI figures out nex…"
---

> Codex adapter: generated from `.claude/commands/game.md`. Use the current user request as invocation arguments. Do not edit this generated skill directly.
...
User invoked `$game [INVOCATION_INPUT]`.
...
4. If `[INVOCATION_INPUT]` is provided — that's intent signal:
...
|---|---|---|
| No `wiki/` and `[INVOCATION_INPUT]` describes idea | **New project** | Invoke `start` skill with description |
...
| `[INVOCATION_INPUT]` contains "mvp" / "доведи до релиза" / "готовь к подаче" / "прогон" | **MVP autonomous pipeline** | Invoke `mvp-to-yandex` skill (analyze + expand к 7-day retention + build + documents) |
| `[INVOCATION_INPUT]` contains "auto release" / "до зелёного" / "автономный релиз" / similar | **Hands-free release loop** | Invoke `auto-release` skill (uses `/goal` v2.1.139+) |
...
If user's `[INVOCATION_INPUT]` overrides detection — respect them. User signals win over inference.
...
If after Step 2 you can't pick a clear path AND `[INVOCATION_INPUT]` describes a capability ("integrate Stripe", "add Discord bot", "OAuth login"):
...
- ❌ Don't ignore `[INVOCATION_INPUT]` — that's the user's explicit signal
