---
name: do
description: "Advisor that ACTS. Reads project context, picks the right skill from the advisor catalog, and EXECUTES it on the project — applies changes, keeps working. Never hands back a copy…"
---

> Codex adapter: generated from `.claude/commands/do.md`. Use the current user request as invocation arguments. Do not edit this generated skill directly.
...
User invoked `$do [INVOCATION_INPUT]`.
...
New project (no `wiki/`) → skip, work from `[INVOCATION_INPUT]`.
...
"add OAuth"), run `node scripts/search-skills.mjs "[INVOCATION_INPUT]"` first; strong match (≥70) → that
...
- ❌ Don't ask "что хочешь сделать?" when context + `[INVOCATION_INPUT]` make it clear.
