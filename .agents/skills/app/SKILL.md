---
name: app
description: "Smart router for app projects. Auto-detects what to do based on context — new app / analyze existing / continue work / UI redesign / pre-release. One command, AI figures out next…"
---

> Codex adapter: generated from `.claude/commands/app.md`. Use the current user request as invocation arguments. Do not edit this generated skill directly.
...
User invoked `$app [INVOCATION_INPUT]`.
...
|---|---|---|
| No `wiki/` and `[INVOCATION_INPUT]` describes idea | **New project** | Invoke `start` skill, then `category-foundation` skill matching detected category |
...
User's `[INVOCATION_INPUT]` always override inferred intent.
...
If after Step 3 you can't pick a clear path AND `[INVOCATION_INPUT]` describes a capability that doesn't fit obvious app-* skills ("integrate Stripe", "add OAuth", "embed Calendly"):
