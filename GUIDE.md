# Project Forge v4.68.16 — Полная инструкция по работе
...
Policy-launcher всегда использует GPT-5.6 Sol на Standard tier, ограничивает фазу двумя субагентами и записывает объявленную модель в `wiki/phases/phase-N.json`. `high` применяется к анализу, дизайну, реализации, визуалу, интеграциям и QA; детерминированный листинг, штатная упаковка релиза и обычные метрики идут на Sol `medium`. Новая задача на каждую фазу не тащит многомиллионный старый контекст. Max/Ultra остаются только ручным решением. Таблица фаз и эскалаций: `.claude/skills/status/references/MODEL-ROUTING.md`.
