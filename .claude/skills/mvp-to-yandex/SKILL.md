...
name: mvp-to-yandex
kind: tactical
description: "Autonomous end-to-end workflow для MVP → Yandex Games submission-ready state. One command, не stop'ит до GREEN. Analyzes MVP, expands к 7-day retention (genre-aware), integrates aggressive ad monetization (interstitial + rewarded х2/boosters/hard currency), localizes RU+EN+TR, builds production yandex zip, generates all documents + art prompts. Uses /goal for autonomous loop. Only asks для critical decisions (missing credentials, genre ambiguous, conflicts). Triggers on: mvp, mvp to yandex, mvp до релиза, mvp pipeline, доведи до релиза, autonomous mvp, готовь к подаче, прогон, full pipeline mvp."
---
