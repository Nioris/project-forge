...
name: localize
kind: tactical
description: "Локализация: режим АРХИТЕКТУРА (дефолт конвейера — словарь I18N.ru + t(), хардкод = дефект, черновик ТОЛЬКО русский по доктрине RU-only) и режим ДОБАВИТЬ ЯЗЫК (только по явной команде /localize <lang>: перевод словаря + скрины + листинг + язык в черновик, по одному языку). Triggers on: localize, локализация, перевод, i18n, словарь строк, добавить язык, english version, translate."
---
