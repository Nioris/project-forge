...
name: app-data-model
kind: architectural
description: "Architectural skill — заложить data model для приложения с самого начала. Schema, persistence, sync, migrations. Без этого retrofit стоит дни. Default: localStorage + IndexedDB для local-first; cloud sync optional. Triggers on: data model, schema, persistence, storage, IndexedDB, migrations, sync data, модель данных, хранилище."
---
