...
kind: architectural
description: "Standardized backend stack decision for app/game projects. Walks user through 4 questions (persistence needs, real-time, RPS, hosting preference), outputs one of 5 canonical stacks with deployment guide. Prevents 'каждый раз решаем заново' syndrome. Triggers on: 'выбрать стек', 'какой backend', 'нужен сервер', 'server stack', 'backend choice', 'choose backend', 'deployment stack'."
---
