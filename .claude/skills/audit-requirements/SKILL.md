...
kind: tactical
description: "Check whether Yandex Games requirements changed since Forge's last audit, and if so, what to re-verify. Fetches the live release-notes + requirements page, compares against the baseline date recorded in this skill, and reports: (a) no change → no audit needed, or (b) changed → here are the new/changed requirement points and which Forge checks (debugcheck, verifiers, fix-moderation map) to update. Triggers on: audit requirements, проверить требования яндекс, требования изменились, yandex requirements changed, нужен ли аудит, release notes, обновились ли требования."
---
