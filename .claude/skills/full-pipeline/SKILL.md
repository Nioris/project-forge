...
name: full-pipeline
kind: tactical
description: "Полный пайплайн Яндекс Игр: от сырой игры до 3 ZIP-билдов. Фаза 1 (полировка) → Фаза 2 (SDK + 13 языков) → Фаза 3 (тест + сборка). Triggers on: full pipeline, полный пайплайн, весь пайплайн, от начала до конца, все фазы."
---
...
3. **VERIFY BEFORE CLAIMING DONE.** Run `bash scripts/verify.sh` before saying "ready".
4. **PHASES ARE ISOLATED.** Phase 1 never touches SDK/localization. Phase 2 never breaks Phase 1 gameplay.
