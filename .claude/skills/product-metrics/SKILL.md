...
kind: architectural
description: "Сгенерировать целевые метрики продукта (D1/D7/D30 retention, ARPU, conversion, session length, north-star) ДО геймдизайна. Использует web research для бенчмарков жанра + платформы. Output: wiki/architecture/metrics.md + wiki/decisions/{NNN}-product-metrics.md. Дальнейшие skills (game-design, monetization-design, improve, deepen-game) сверяются с этими таргетами. Triggers on: метрики, KPI, retention, ARPU, target metrics, бенчмарки, цели проекта, north-star metric."
---
