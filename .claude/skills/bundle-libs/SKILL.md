...
name: bundle-libs
kind: tactical
description: "Bundle external CDN libraries into game zip для Yandex Games. Scans HTML/JS for external <script src=https://...>, downloads to assets/lib/, replaces refs с local paths. Yandex Games sandbox blocks external HTTP — all libs must be bundled. Triggers on: bundle libs, скачай либы, sandbox compliance, нужно убрать cdn, no external scripts, локализуй либы, embed dependencies, оффлайн билд."
---
