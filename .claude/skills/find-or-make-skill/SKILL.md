...
kind: tactical
description: 'Before Claude attempts any specialized task (game physics, animation library, specific SDK, design system, monetization strategy, etc) — use this skill to find an existing skill that covers the task BEFORE hallucinating a solution. Checks local .claude/skills/, Anthropic''s official marketplace, community marketplaces, and web search. Only if nothing fits — falls back to /write-skill to create one. Use when user asks for specialized work and no relevant skill is obviously loaded. Trigger phrases: "найди скил", "есть ли скил", "find a skill", "what skill should I use", or implicitly when Claude is about to work on something outside standard code (design, monetization, specific library integration).'
---
