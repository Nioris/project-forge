...
kind: tactical
description: "Integrate Yandex AppMetrica analytics SDK в Android wrapper. Auto-detects wrapper type (TWA, Capacitor, Cordova, Native Kotlin/Java) by project files, applies correct integration pattern. Sets up dependency, manifest meta-data, activation code, optional JS bridge (для WebView). Validates через check-appmetrica.mjs. Used during RuStore release prep — analytics is de-facto mandatory для RuStore featured consideration. Triggers on: appmetrica, app metrica, аналитика, analytics integration, push notifications, в рустор аналитику, integrate analytics."
---
