...
name: auto-release
kind: tactical
description: "Autonomous release workflow via Claude Code /goal (v2.1.139+). Sets persistent goal 'release-ready returns GREEN for {platform}', Claude iterates fix→validate→fix until все validators pass, then stops. Use для unattended release prep. Triggers on: автоматически релиз, auto release, до зелёного, until green, релиз без вопросов, не отвлекай меня релиз, автономный релиз, /goal релиз, /goal release, hands-free release."
---
