...
name: ui-review
kind: tactical
description: "Screenshot-driven UI audit. AI vision systematically scans interface zone-by-zone detecting collisions, overlaps, prejat'ie, illegible labels, broken hierarchy. Output — structured violations list с координатами + source files + concrete CSS fixes. NOT principles-based (no more 'improve spacing' generic advice). Mandatory screenshot input — without it skill refuses или requests auto-snapshot. Triggers on: review, audit, отзыв, проверь UX, проверь UI, ui review, design review, проверь интерфейс, посмотри на скриншот, что не так с UI, collisions, налипают, перекрывают, плохо выглядит, кривой UI."
---
...
## 📸 САМООЦЕНКА ПО КАДРАМ (обязательный внутренний шаг, до независимой приёмки)
...
### Процедура
```
node <движок>/scripts/screens-shoot.mjs .
```
Скрипт берёт утверждённый список из `wiki/design/screen-flow.json`, переключает реальное
состояние через `window.__FORGE_VISUAL_QA__` и снимает его на **мобильном 412** и десктопе 1920,
...
Есть `assets/target/screens/manifest.json` — каждый live-кадр сравнивается со своим созданным
GPT Image mobile/desktop blueprint; общий `assets/target/target-frame.png` задаёт style DNA.
Оценка каждого экрана дополняется строкой:
...
- любой экран **ниже 7/10 → в работу**, показывать пользователю рано; 8/10 — целевой внутренний уровень, не повод округлять оценку вверх;
...
### Самооценка не закрывает Phase 4

Builder использует этот раздел, чтобы находить и исправлять собственные ошибки. Для durable
`Phase 4 complete` после него всё равно нужен **другой reviewer session**, который откроет каждый
PNG из capture manifest и создаст канонический visual review/evidence по `/visual-qa`. Нельзя
подменять визуальную рецензию проверкой DOM, отсутствием console errors или фразой «выглядит
готово»: центральный `phase4-visual-evidence` verifier это отклонит.
