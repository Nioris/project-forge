...
name: convert
kind: tactical
description: Convert HTML5 project to Android APK/AAB for RuStore. Full pipeline from analysis to build. Use when user says "convert", "конвертируй", "в apk", "android", "rustore", "apk из html".
---

# Convert HTML5 → Android

## Arguments
`$ARGUMENTS`: project folder path (or empty = scan for first unprocessed)
