...
kind: architectural
description: "Фаза 8 — выпуск по явно выбранным storefront targets: проверяет target manifest, локальные кандидаты и external delivery boundary. До установленного target-specific verifier фаза остаётся на STOP, а не имитирует submit-ready. Триггеры: фаза 8, phase 8, релизная фаза, собрать и подать."
contract_version: 1
phases:
  - 8
modes:
