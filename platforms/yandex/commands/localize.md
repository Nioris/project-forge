...
6. **Автоопределение (п. 2.14):** через `ysdk.environment.i18n.lang` (язык портала), НЕ `browser.lang` и НЕ `navigator.language`.
7. **detectLang() ПЕРЕД LoadingAPI.ready()** — модерация проверяет: при открытии на турецком домене UI сразу на турецком.
...
function detectLang() {
  // 1. URL параметр (для локального тестирования: ?lang=es)
  //    На платформе Яндекс: &lang=xx (п. 2.14, документация)
  const urlLang = new URLSearchParams(location.search).get('lang');
  if (urlLang && I18N[urlLang]) { _lang = urlLang; return; }

  // 2. SDK (обязательно по п. 2.14 требований — автоопределение через SDK)
...
□ detectLang() вызывается ДО LoadingAPI.ready() (п. 2.14)
