...
### 🔴 #1: GameReady API вызывается НЕ ВОВРЕМЯ (п. 1.19)
**Самый частый реджект.** Модерация видит что `LoadingAPI.ready()` вызывается ПОСЛЕ того как игра уже доступна, или ДО того как UI готов.
...
// 3. Отрисовать первый кадр (title screen ВИДИМ на экране)
renderTitleScreen();
// 4. ТОЛЬКО ТЕПЕРЬ:
ysdk.features.LoadingAPI?.ready();
console.log('[SDK] LoadingAPI.ready() — title screen visible');
```

**ОШИБКИ:**
- ❌ `ready()` в `DOMContentLoaded` (UI ещё не отрисован)
- ❌ `ready()` до загрузки шрифтов (текст прыгает)
- ❌ `ready()` после того как игрок уже нажал Play
- ✅ `ready()` когда title screen полностью видим и кликабелен
