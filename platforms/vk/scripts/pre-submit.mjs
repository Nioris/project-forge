...
/**
 * @file pre-submit.mjs
 * @description VK Mini Apps pre-submit gate. Runs 3 specialised validators.
 *              Companion to the legacy `scripts/verify-vk.mjs` at repo root —
 *              the two are complementary (verify-vk covers general structure,
 *              this one catches specific Bridge/Pay/Ads pitfalls).
 *
...
async function main() {
  ...
  if (pos.length === 0) {
    console.error('Usage: node platforms/vk/scripts/pre-submit.mjs <gamePath> [--verbose]');
    process.exit(2);
  }
  ...
  const bar = '=========================================================';
  console.log(bar);
  console.log('  VK PRE-SUBMIT: ' + path.basename(gamePath));
  console.log(bar);
  ...
  fs.writeFileSync(
    path.join(gamePath, '.pre-submit-vk.json'),
    JSON.stringify({ platform: 'vk', blockers: b, warnings: w, fatals: f, results, timestamp: new Date().toISOString() }, null, 2)
  );
