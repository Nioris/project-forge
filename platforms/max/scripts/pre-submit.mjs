...
/**
 * @file pre-submit.mjs
 * @description MAX Mini App pre-submit gate. Runs 5 validators.
 *              Same exit-code contract as other platforms.
 *
 *   Usage:
...
const VALIDATOR_FILES = [
  ...
  'url-constraints.mjs',
  'https-only.mjs',
  'initdata-and-conflict.mjs',
  'gesture-required.mjs',
];
...
function printReport(gamePath, results, summary, opts) {
  ...
  const bar = '=========================================================';
  console.log(bar);
  console.log('  MAX PRE-SUBMIT: ' + name);
  ...
  for (const r of results) {
    ...
    console.log(status + '  ' + r.validator.ID.padEnd(22) + '  blockers=' + b + '  warnings=' + w);
    for (const issue of r.issues) {
  ...
  if (summary.blockers > 0) {
    ...
    console.log('READY for MAX deploy (' + summary.warnings + ' warnings — review before submitting to business.max.ru).');
  }
  console.log(bar);
}
...
async function run(gamePath, opts) {
  ...
  fs.writeFileSync(
    path.join(path.resolve(gamePath), '.pre-submit-max.json'),
    JSON.stringify({
      platform: 'max',
...
if (positional.length === 0) {
  console.error('Usage: node platforms/max/scripts/pre-submit.mjs <gamePath> [--verbose]');
  process.exit(2);
}
