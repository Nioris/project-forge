#!/usr/bin/env node
/**
 * @file pre-submit.mjs
 * @description OK (Одноклассники) Mini App pre-submit gate.
 *              Same contract as Yandex and Telegram adapters.
 *
 *   Usage:
 *     node platforms/ok/scripts/pre-submit.mjs WorkProgress/{Project}/
 */

import path from 'node:path';
import fs from 'node:fs';
import { fileURLToPath } from 'node:url';

const here = path.dirname(fileURLToPath(import.meta.url));
const validatorsDir = path.resolve(here, '..', 'validators');
const VALIDATOR_FILES = ['fapi-sdk.mjs'];
const LEVELS = { BLOCKER: 'blocker', WARNING: 'warning', INFO: 'info' };

async function run(gamePath, opts) {
  if (!fs.existsSync(gamePath) || !fs.statSync(gamePath).isDirectory()) {
    console.error('Not a directory: ' + gamePath);
    process.exit(2);
  }

  const results = [];
  for (const file of VALIDATOR_FILES) {
    const full = path.join(validatorsDir, file);
    if (!fs.existsSync(full)) continue;
    const mod = await import('file://' + full.replace(/\\/g, '/'));
    try {
      const issues = mod.validate(gamePath);
      results.push({ validator: { ID: mod.ID }, ok: true, issues });
    } catch (e) {
      results.push({ validator: { ID: mod.ID || file }, ok: false, error: e.message });
    }
  }

  let b = 0, w = 0, f = 0;
  for (const r of results) {
    if (!r.ok) { f++; continue; }
    for (const i of r.issues) {
      if (i.level === LEVELS.BLOCKER) b++;
      else if (i.level === LEVELS.WARNING) w++;
    }
  }

  const bar = '=========================================================';
  console.log(bar);
  console.log('  OK PRE-SUBMIT: ' + path.basename(path.resolve(gamePath)));
  console.log(bar);
  console.log('  TOTAL: ' + b + ' blockers, ' + w + ' warnings, ' + f + ' fatals');
  console.log('');

  for (const r of results) {
    if (!r.ok) { console.log('[FATAL] ' + r.validator.ID + ' — ' + r.error); continue; }
    const bb = r.issues.filter(i => i.level === LEVELS.BLOCKER).length;
    const ww = r.issues.filter(i => i.level === LEVELS.WARNING).length;
    const status = bb > 0 ? '[X]' : ww > 0 ? '[!]' : '[OK]';
    console.log(status + ' ' + r.validator.ID + '  b=' + bb + ' w=' + ww);
    for (const issue of r.issues) {
      if (!opts.verbose && issue.level !== LEVELS.BLOCKER) continue;
      const sym = issue.level === 'blocker' ? '[X]' : '[!]';
      console.log('    ' + sym + ' [' + issue.id + '] ' + issue.message);
    }
  }

  console.log('');
  console.log(b > 0 ? 'BLOCKED — fix ' + b + ' blocker(s).' : f > 0 ? 'VALIDATOR ERRORS' : 'READY');
  console.log(bar);

  fs.writeFileSync(
    path.join(path.resolve(gamePath), '.pre-submit-ok.json'),
    JSON.stringify({ platform: 'ok', blockers: b, warnings: w, fatals: f, results }, null, 2)
  );

  process.exit(b > 0 || f > 0 ? 1 : 0);
}

const args = process.argv.slice(2);
const pos = args.filter(a => !a.startsWith('-'));
const opts = { verbose: args.includes('--verbose') || args.includes('-v') };
if (pos.length === 0) {
  console.error('Usage: node platforms/ok/scripts/pre-submit.mjs <gamePath>');
  process.exit(2);
}
run(pos[0], opts);
