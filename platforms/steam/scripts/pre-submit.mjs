...
/**
 * @file pre-submit.mjs
 * @description Steam pre-submit gate. Runs 5 specialised validators against
 *              an Electron+steamworks.js HTML5 game directory.
 *
...
async function main() {
  ...
  if (pos.length === 0) {
    console.error('Usage: node platforms/steam/scripts/pre-submit.mjs <gamePath> [--verbose]');
    process.exit(2);
  }
  ...
  const bar = '=========================================================';
  console.log(bar);
  console.log('  STEAM PRE-SUBMIT: ' + path.basename(gamePath));
  console.log(bar);
  ...
  fs.writeFileSync(
    path.join(gamePath, '.pre-submit-steam.json'),
    JSON.stringify({ platform: 'steam', blockers: b, warnings: w, fatals: f, results, timestamp: new Date().toISOString() }, null, 2)
  );
