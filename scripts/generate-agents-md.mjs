#!/usr/bin/env node
/**
 * @file generate-agents-md.mjs
 * @description Generates concise Codex instructions for both the Forge engine root
 *              and synced sibling projects. FORGE.md is the agent-neutral runtime contract. CLAUDE.md remains the engine's complete
 *              architectural source of truth; neither generated file replaces it.
...
const hash = createHash('sha256').update(claude).digest('hex').slice(0, 16);
const skillCount = readdirSync('.claude/skills', { withFileTypes: true }).filter(e => e.isDirectory()).length;
const agentCount = readdirSync('.claude/agents', { withFileTypes: true }).filter(e => e.isFile() && e.name.endsWith('.md')).length;
const canonicalNames = new Set(readdirSync('.claude/skills', { withFileTypes: true }).filter(e => e.isDirectory()).map(e => e.name));
const routerCount = readdirSync('.claude/commands').filter(f => f.endsWith('.md') && !canonicalNames.has(f.replace(/\.md$/i, ''))).length;

const common = `## Critical workspace discipline
...
- ${skillCount} canonical Forge skills are shipped under \`.claude/skills/<name>/SKILL.md\`. Codex gets those plus ${routerCount} generated smart-router skills from \`.claude/commands/\`, for ${skillCount + routerCount} discoverable skills under \`.agents/skills/\`.
...
<!-- GENERATED from Forge canonical sources; claude-hash:${hash}; target:engine; do not edit by hand. -->
...
const projectDoc = `# Project Forge v${plugin.version} — Codex project runtime instructions

<!-- GENERATED from Forge canonical sources; claude-hash:${hash}; target:project; do not edit by hand. -->
...
writeFileSync('AGENTS.md', engineDoc, 'utf8');
writeFileSync('AGENTS.project.md', projectDoc, 'utf8');
console.log(`AGENTS files generated (engine ${Buffer.byteLength(engineDoc)} bytes, project ${Buffer.byteLength(projectDoc)} bytes, CLAUDE hash ${hash}, ${skillCount} canonical skills + ${routerCount} routers, ${agentCount} agents)`);
