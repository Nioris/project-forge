...
**Текущая публичная версия:** `v4.68.13`
...
Он запускает новую задачу Codex на Standard tier с моделью и reasoning по фазе: Terra выполняет основную техническую работу, Sol включается для дизайна и сложных эскалаций, Luna — только для механических операций. `$phase-*` внутри уже открытой задачи не переключает её основную модель. Полная таблица и доступные `--route`: `.claude/skills/status/references/MODEL-ROUTING.md`.
...
В `v4.68.13` обычная авторизация и API-профили остаются разделены.
...
- [RELEASE_NOTES_v4.68.13.md](RELEASE_NOTES_v4.68.13.md) — изменения текущей версии
