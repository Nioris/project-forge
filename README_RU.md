...
**Текущая публичная версия:** `v4.68.16`
...
Он запускает новую задачу Codex на Standard tier: все фазы и подагенты используют GPT-5.6 Sol, а reasoning зависит от работы. `high` — анализ/дизайн/реализация/визуал/техника/QA; `medium` — детерминированный листинг, штатная упаковка и обычные метрики. `$phase-*` внутри уже открытой задачи не переключает её основную модель и сохраняет дорогой старый контекст. Полная таблица и доступные `--route`: `.claude/skills/status/references/MODEL-ROUTING.md`.
...
В `v4.68.16` обычная авторизация и API-профили остаются разделены.
